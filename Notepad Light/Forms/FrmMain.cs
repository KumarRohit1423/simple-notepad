    ...
    public partial class FrmMain : Form
    {
        ...
        public void WriteFileContents()
        {
            try
            {
                ...
                RtbPage.Invoke((System.Windows.Forms.MethodInvoker)delegate { if (RtbPage.Modified == false) { return; } });

                // save the existing changes to the file
                if (gCurrentFileName != Strings.defaultFileName)
                {
                    // need to avoid cross thread operation
                    RtbPage.Invoke((System.Windows.Forms.MethodInvoker)delegate { FileSave(); });
                }
            }
            catch (Exception ex)
            {
